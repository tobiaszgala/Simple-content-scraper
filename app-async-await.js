...
const getData = async (url) => {
    // get shirt uri
    const shirtURI = await ezscraper(url, {
        link: {
    ...
    // get all shirt links
    const allShirtLinks = await ezscraper(url + shirtURI.link[0].link, {
        link: {
            selector: 'ul.products a',
            property: 'href'
        }
    });
    // create array of promises and add additional property 'url'
    const arrOfPromises = allShirtLinks.link.map(item => ezscraper(url + item.link, {
